...
initialize_vector_tables() {
    # Check if the SQL file for creating vector tables exists
    if [ -f "$(dirname "$0")/initialize_vector_tables.sql" ]; then
        PGPASSWORD=$DB_PASSWORD psql -U $DB_USER -h $DB_HOST -p $DB_PORT -d $DB_NAME -f "$(dirname "$0")/initialize_vector_tables.sql"
        echo "Vector tables initialized successfully from initialize_vector_tables.sql."
    else
        echo "initialize_vector_tables.sql file not found in the expected directory." >&2
        exit 1
    fi
}
